...
function qrConversionErrorMessage(error: unknown, errorCode?: string | null): string {
  ...
  useEffect(() => {
    ...
    fetch(`/api/rates?from=${selectedCoin.symbol}&to=${customerCoin.symbol}&chainId=${paymentChainId}`, { signal: ctrl.signal })
      .then(async r => {
        ...
        if (!r.ok) throw Object.assign(new Error(data.detail || data.error || ""), { errorCode: data.errorCode });
        return data;
      })
      .then(data => {
        if (data.rate) {
            ...
            {conversionError ? (
              <div
                ...
                style={{
                  ...
                  fontWeight: 650,
                  lineHeight: 1.45,
                  // Centred: this banner sits directly above the QR on a
                  // narrow, mostly-mobile card, so left-aligned text reads as
                  // misaligned against the centred amount and QR.
                  textAlign: "center",
                }}
              >
                {conversionError}
    ...
    <div style={{ minHeight: "100dvh", background: "#F2F2F7", fontFamily: font }}>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "24px 16px 40px", minHeight: "calc(100dvh - 58px)", display: "grid", alignContent: "center", boxSizing: "border-box" }}>
        ...
        */}
        ...
        <a
          href="https://github.com/sera-cx/sera-pay"
          target="_blank"
          rel="noopener noreferrer"
          className="serapay-oss-link"
          style={{
            marginTop: 16,
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          <svg className="serapay-oss-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0, opacity: 0.55 }}>
