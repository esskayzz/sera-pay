...
import type { QrCanvasRuntime } from "./qr-card";
...
function getLogoImageData(image: HTMLImageElement, createCanvas: QrCanvasRuntime["createCanvas"], sampleSize = 224) {
...
function drawAdvancedQrCanvas({
  ...
  const qr = QRCodeGenerator.create(value || " ", { errorCorrectionLevel: "H" });
  ...
  for (let row = 0; row < moduleCount; row += 1) {
    for (let col = 0; col < moduleCount; col += 1) {
      ...
      // Timing/alignment modules need solid coverage for reliable scanning.
      const dotSize = reserved ? moduleSize : moduleSize * Math.min(0.92, baseScale + brandPresence * 0.1 + (seed === 0 ? 0.04 : 0));
      ...
      if (reserved) {
        ...
        const radius = dotSize * (style === "classy-rounded" ? 0.28 : seed % 5 === 0 ? 0.34 : 0.48);
...
export function buildQrOptions(value: string, size: number, fgColor: string, bgColor: string, style: QrStyle, logo?: string): Options {
  const variants = {
    classic: ["square", "square", "square"],
    rounded: ["extra-rounded", "extra-rounded", "dot"],
    dots: ["dots", "dot", "dot"],
    classy: ["classy", "square", "square"],
    "classy-rounded": ["classy-rounded", "extra-rounded", "dot"],
  } as const;
  const [dots, square, dot] = variants[style] || variants.classic;
  return {
    width: size, height: size, data: value, type: "svg", margin: Math.max(8, Math.round(size * 0.04)),
    backgroundOptions: { color: bgColor },
    dotsOptions: { color: fgColor, type: dots },
    ...
    ...(logo ? { image: logo, imageOptions: { hideBackgroundDots: true, imageSize: 0.28, margin: 3, crossOrigin: "anonymous" } } : {}),
  };
}

export function renderAdvancedQr(canvas: HTMLCanvasElement, value: string, size: number, bgColor: string, style: QrStyle,
  logoImage: HTMLImageElement | undefined, createCanvas: QrCanvasRuntime["createCanvas"], pixelRatio = 1) {
  const imageData = logoImage ? getLogoImageData(logoImage, createCanvas) : null;
  const palette = imageData ? getAutoQrPalette(imageData) : FALLBACK_AUTO_PALETTE;
  drawAdvancedQrCanvas({ canvas, value, size, bgColor, style, logoImage, imageData, palette, pixelRatio });
}
